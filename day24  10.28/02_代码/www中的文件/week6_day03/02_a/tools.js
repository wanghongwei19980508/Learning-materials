...
function getUrlVal() {
    ...
    var str = window.decodeURIComponent(window.location.search).substr(1);

    // 2,判断 是一个参数还是多个参数
    // 参数字符串中,是否有 & 符号
    // 字符串.indexOf('&')    字符串.search('&')   如果没有返回值是-1
    // console.log( str.indexOf('&') );
    // console.log( str.search('&') );


    if (str.indexOf('&') === -1) {
        // 没有 & 是 一个 数据单元 直接按照 等号分为数组,
        // 数组[0] 是 对象的属性 数组[1] 是对象的属性值
        var arr2 = str.split('=');
...
// 参数3 : 运动停止时,执行的回调函数, 可以是匿名函数,或者定义好的函数名称
